...
Sampler sampleBuffer(int idx, float u, float v) {
	...
	//float alpha = fract(u - 0.5f);
	//float beta = fract(v - 0.5f);
	
	//int i = int(floor(fi));
	//int j = int(floor(fj));
	//int l = j * numSamplesU + i;
	//float factorU = fract(fi);
	//float factorV = fract(fj);
	...
	//vec4 p00 = lsDataBuffer.localSurfaceData[idx + l];
	//vec4 p10 = lsDataBuffer.localSurfaceData[idx + l + 1];
	//vec4 p01 = lsDataBuffer.localSurfaceData[idx + l + numSamplesU];
	//vec4 p11 = lsDataBuffer.localSurfaceData[idx + l + numSamplesU + 1];
	//res.p = mix(mix(p00, p10, factorU), mix(p01, p11, factorU), factorV).xyz;
	...
	//res.u = lsDataBuffer.localSurfaceData[idx + l + offsetU].xyz;
	...
	//res.v = lsDataBuffer.localSurfaceData[idx + l + offsetV].xyz;
