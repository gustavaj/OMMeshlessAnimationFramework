...
namespace vks
{
	...
	{
		return ImGui::CollapsingHeader(caption, open ? ImGuiTreeNodeFlags_DefaultOpen : 0);
	}
	...
	bool UIOverlay::inputFloat3(const char* caption, float value[3], int decimal_precision)
	{
		bool res = ImGui::InputFloat3(caption, value, decimal_precision,
	...
	bool UIOverlay::sliderFloat3(const char* caption, float value[3], float min, float max)
	{
	...
	void UIOverlay::frameTimeGraph(float frameTime)
	{
	}
	bool UIOverlay::inputVec3(const char* caption, float value[3], float step, uint32_t precision, std::string num)
	{
		/*float oldFramePaddingY = ImGui::GetStyle().FramePadding.y;
		ImGui::PushItemWidth(75.0f);
		ImGui::GetStyle().FramePadding.y = 5.0f;
		text(("x" + num + ":").c_str());
		ImGui::SameLine(0.0f);
		bool resx = inputFloat(("y" + num + ":").c_str(), &value[0], step, precision);
		ImGui::SameLine(0.0f);
		bool resy = inputFloat(("z" + num + ":").c_str(), &value[1], step, precision);
		ImGui::SameLine(0.0f);
		bool resz = inputFloat(caption, &value[2], step, precision);
		bool res = resx || resy || resz;
		if (res) { updated = true; }
		ImGui::GetStyle().FramePadding.y = oldFramePaddingY;
		ImGui::PopItemWidth();*/
		//return res;
