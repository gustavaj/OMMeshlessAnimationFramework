	...
	NameSpirvPair Shaders::GetLocalSurfaceInfoTescShader(uint32_t patchVertices, LocalSurfaceType lsType, ShaderOptions& options)
	{
		...
		if (Shaders::NotInMap(name))
		{
			std::string src =
				...
				"  for(int i = 0; i < 6; i++) {\n"
				"    if(dot(vec4(pos, 1.0), ubo.frustumPlanes[i]) <= -r) {\n"
				...
				"}\n"
				" \n"
				"float calcEdgeTessellationSphere(vec3 control0, vec3 control1) {\n"
				"  float e0 = distance(control0, control1);\n"
				"  vec3 m0 = (control0 + control1) / 2;\n"
				"  if(ubo.doCulling) {\n"
				"    if(!sphereInFrustum(vec3(ubo.modelview * vec4(m0, 1.0f)), e0)) {\n"
				"      return 0;\n"
				"    }\n"
				"  }\n"
			...
			if (patchVertices == 4) {
				src +=
					...
					"        vec3 topLeft  = vec3(ubo.modelview * vec4(evaluateLocal(tcLSInfo[0], 0.0, 0.0).p, 1.0f));\n"
					"        vec3 topRight = vec3(ubo.modelview * vec4(evaluateLocal(tcLSInfo[1], 1.0, 0.0).p, 1.0f));\n"
					"        vec3 botLeft  = vec3(ubo.modelview * vec4(evaluateLocal(tcLSInfo[2], 0.0, 1.0).p, 1.0f));\n"
					"        vec3 botRight = vec3(ubo.modelview * vec4(evaluateLocal(tcLSInfo[3], 1.0, 1.0).p, 1.0f));\n"
					"        vec3 center = (topLeft + topRight + botLeft + botRight) / 4.0f;\n"
					"        float radius = max(max(distance(topLeft, center), distance(topRight, center)),\n"
					"          max(distance(botLeft, center), distance(botRight, center)));\n"
					...
					"      float tf0 = calcEdgeTessellationSphere(topLeft, botLeft);\n"
					"      float tf1 = calcEdgeTessellationSphere(topLeft, topRight);\n"
					"      float tf2 = calcEdgeTessellationSphere(topRight, botRight);\n"
					"      float tf3 = calcEdgeTessellationSphere(botLeft, botRight);\n"
