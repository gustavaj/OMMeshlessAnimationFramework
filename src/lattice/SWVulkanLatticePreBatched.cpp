...
namespace SWVL
{
	...
	SWVulkanLatticePreBatched::SWVulkanLatticePreBatched()
		: SWVulkanLatticePreBatched("")
	{
	}

	SWVulkanLatticePreBatched::SWVulkanLatticePreBatched(std::string name)
	...
	void SWVulkanLatticePreBatched::initVulkanStuff(VkDevice* device, vks::VulkanDevice* vulkanDevice, VkQueue* queue, VkCommandPool* commandPool, VkDescriptorPool* descriptorPool, VkRenderPass* renderPass, VkAllocationCallbacks* allocator)
	{
	...
	void SWVulkanLatticePreBatched::destroyVulkanStuff()
	{
		...
		if (m_pointsPipeline) {
			...
			vkDestroyDescriptorSetLayout(*m_device, m_batchDescSetLayout, m_allocator);
		}
	...
	void SWVulkanLatticePreBatched::addToCommandbufferPreRenderpass(VkCommandBuffer& commandBuffer)
	{
		if (!m_draw) return;
	...
	void SWVulkanLatticePreBatched::addToCommandbuffer(VkCommandBuffer& commandBuffer)
	{
		if (!m_draw) return;
	...
	void SWVulkanLatticePreBatched::onViewChanged(glm::mat4 projection, glm::mat4 view)
	{
	...
	void SWVulkanLatticePreBatched::onWindowResized(float width, float height)
	{
		m_uniforms.windowSize = glm::vec2(width, height);
	}

	void SWVulkanLatticePreBatched::localUpdate(double dt)
	{
		if (m_simulate) {
	...
	bool SWVulkanLatticePreBatched::onUpdateUIOverlay(vks::UIOverlay* overlay)
	{
	...
	void SWVulkanLatticePreBatched::CheckAndSetupRequiredPhysicalDeviceFeatures(
	...
	void SWVulkanLatticePreBatched::createDeviceLocalBuffer(VkBuffer& buffer, VkDeviceMemory& memory, void* data, uint32_t bufferSize, VkBufferUsageFlagBits usage)
	{
	...
	void SWVulkanLatticePreBatched::setupVertices()
	{
		...
		for (size_t i = 0; i < m_patches.size(); i++)
		{
			if (i % NUM_PATCHES_PER_BATCH == 0) {
				batch++;
				// TODO: If the alst batch can not fill up all the spaces, make it smaller!
				m_batchTextures.push_back(LocalSurfaceTextureBatch(
					BATCH_ROWS, BATCH_COLS, NUM_SAMPLES_U_BATCH, NUM_SAMPLES_V_BATCH,
			...
			m_patchVertices.push_back(std::move(localVert00));
			m_patchVertices.push_back(std::move(localVert10));
			m_patchVertices.push_back(std::move(localVert01));
			m_patchVertices.push_back(std::move(localVert11));
		}
		...
		m_batchDescSets.resize(m_batchInfo.size());
	...
	void SWVulkanLatticePreBatched::createBuffers()
	{
		// Lattice grid points
	...
	void SWVulkanLatticePreBatched::prepareUniformBuffers()
	{
	...
	void SWVulkanLatticePreBatched::setupDescriptorSetLayouts()
	{
		...
		descriptorLayout = vks::initializers::descriptorSetLayoutCreateInfo(
			setLayoutBindings.data(), static_cast<uint32_t>(setLayoutBindings.size()));
		VK_CHECK_RESULT(vkCreateDescriptorSetLayout(*m_device, &descriptorLayout,
			m_allocator, &m_batchDescSetLayout));
		descriptorLayouts.push_back(m_batchDescSetLayout);
	...
	VkPipelineShaderStageCreateInfo SWVulkanLatticePreBatched::loadShader(std::string fileName, VkShaderStageFlagBits stage)
	{
	...
	void SWVulkanLatticePreBatched::setupDescriptorPool()
	{
		std::vector<VkDescriptorPoolSize> poolSizes =
		{
			...
			vks::initializers::descriptorPoolSize(VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, MAX_BATCHES)
		};
	...
	void SWVulkanLatticePreBatched::setupDescriptorSets()
	{
		...
		for (size_t i = 0; i < m_batchDescSets.size(); i++)
		{
			allocInfo = vks::initializers::descriptorSetAllocateInfo(*m_descriptorPool, &m_batchDescSetLayout, 1);
			VK_CHECK_RESULT(vkAllocateDescriptorSets(*m_device, &allocInfo, &m_batchDescSets[i]));
	...
	void SWVulkanLatticePreBatched::updateLatticeUniformBuffer()
	{
	...
	void SWVulkanLatticePreBatched::updateMatrixUniformBuffer()
	{
		memcpy(m_matrixUniformBuffer.mapped, &m_matrices[0][0], sizeof(glm::mat4) * m_matrices.size());
	}

	void SWVulkanLatticePreBatched::uploadStorageBuffers()
	{
	...
	void SWVulkanLatticePreBatched::setupQueryResultBuffer()
	{
	...
	void SWVulkanLatticePreBatched::getQueryResults()
	{
		if (m_doPipelineQueries) {
