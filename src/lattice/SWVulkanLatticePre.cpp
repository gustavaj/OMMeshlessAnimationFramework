...
namespace SWVL
{
	...
	VkPipelineShaderStageCreateInfo SWVulkanLatticePre::loadShader(std::pair<std::string, std::vector<uint32_t>&> src, VkShaderStageFlagBits stage)
	{
		VkPipelineShaderStageCreateInfo shaderStage = {};
		shaderStage.sType = VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO;
		shaderStage.stage = stage;
	...
	void SWVulkanLatticePre::preparePipelines()
	{
		...
		options.numSamplesU = NUM_SAMPLES_U_IMG;
		options.numSamplesV = NUM_SAMPLES_V_IMG;
